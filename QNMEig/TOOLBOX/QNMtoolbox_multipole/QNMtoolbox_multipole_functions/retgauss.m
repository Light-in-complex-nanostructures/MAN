...
% si z=f(xx), l'integrale definie de a à xint de f est C*z(:); vecteur colonne  de meme longueur que xint, C:matrice pleine
% 
...
%  si z=f(xx), l'integrale definie de a à xint de f est P*cumsum(wx(:).*z(:))+C*z(:); vecteur colonne
%
...
%  figure;semilogy(xint,abs(y-cos(a)+cos(xint)),'-',x,abs(yy-cos(a)+cos(x)),'--r');retlegend('avec correction','sans correction',3);title(rettexte(a,b));
%%
%%% TEST
